...
void iotx_mc_set_client_state(iotx_mc_client_t *pClient, iotx_mc_state_t newState)
{
    ...
    /*---------------------- jsq -----------------------------*/
    if(pClient->client_state == IOTX_MC_STATE_DISCONNECTED){
        if(pClient->disconnected_callback){
            pClient->disconnected_callback();
        }
    }
    /*---------------------- jsq -----------------------------*/
}
...
// jsq add
#define TRY_RESUBSCRIBE_MAX     3
static int mqtt_resubscribe_all_topic(iotx_mc_client_t *pclient)
{
    iotx_mc_topic_handle_t *node;
    //int try_cnt = 0;
    //int rc;
    ...
    if(pclient == NULL){
        mqtt_err("mqtt client is NULL.\n");
        return FAIL_RETURN;
    }

    if(list_empty(&pclient->list_sub_handle)){
        printf("subscribe handle list is empty.\n");
        return SUCCESS_RETURN;
    }

    list_for_each_entry(node, &pclient->list_sub_handle, linked_list, iotx_mc_topic_handle_t){
        if((node->topic_filter == NULL) || (node->handle.h_fp == NULL)){
            continue;
        }

        wrapper_mqtt_subscribe(pclient,
                            node->topic_filter,
                            node->qos,
                            node->handle.h_fp,
                            node->handle.pcontext);
    }
    return SUCCESS_RETURN;
}
// jsq add
...
int wrapper_mqtt_release(void **c)
{
    ...
    if(pClient->buf_host_name != NULL){
        mqtt_free(pClient->buf_host_name);
        pClient->buf_host_name = NULL;
    }
    if(pClient->buf_client_id != NULL){
        mqtt_free(pClient->buf_client_id);
        pClient->buf_client_id = NULL;
    }
    if(pClient->buf_username != NULL){
        mqtt_free(pClient->buf_username);
        pClient->buf_username = NULL;
    }
    if(pClient->buf_password != NULL){
        mqtt_free(pClient->buf_password);
        pClient->buf_password = NULL;
    }
    if(pClient->buf_will_topic != NULL){
        mqtt_free(pClient->buf_will_topic);
        pClient->buf_will_topic = NULL;
    }
    if(pClient->buf_will_message != NULL){
        mqtt_free(pClient->buf_will_message);
        pClient->buf_will_message = NULL;
    }

    pClient->connect_success_callback = NULL;
    pClient->disconnected_callback = NULL;
    // jsq add
    mqtt_free(pClient);
#else
    memset(pClient, 0, sizeof(iotx_mc_client_t));
...
int wrapper_mqtt_connect(void *client)
{
    ...
    // jsq add 
    if(rc == SUCCESS_RETURN){
        if(pClient->connect_success_callback){
            pClient->connect_success_callback();
        }

        mqtt_resubscribe_all_topic(pClient);
    }
    // jsq add 
