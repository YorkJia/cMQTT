...
static int user_connect_success_callback(void)
{
    HAL_Printf("connect broker success callback.\n");
}
...
void *IOT_MQTT_Construct(iotx_mqtt_param_t *pInitParams)
{
   ...
   void *callback;
    ...
    if (pInitParams->keepalive_interval_ms < CONFIG_MQTT_KEEPALIVE_INTERVAL_MIN * 1000 ||
        ...
        mqtt_params.keepalive_interval_ms = CONFIG_MQTT_KEEPALIVE_INTERVAL_MIN * 1000;
    } else {
...
/*----------------- jsq add ---------------------------*/
int IOT_MQTT_Connect(void *pclient)
{
    if (pclient == NULL){
